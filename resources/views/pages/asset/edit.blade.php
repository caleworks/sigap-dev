                        ...
                        <div class="col-md-8">
                            <div class="row m-3">
                                ...
                                <input type="text" name="asset_code" id="asset_code" class="form-control @error('asset_code') is-invalid @enderror" 
                                    placeholder="Example: fncltp01" value="{{ old('asset_code', $assetDetail->asset_code) }}" required>
                                @error('asset_code')
                            ...
                            <div class="row m-3">
                                ...
                                <input type="text" name="asset_name" id="asset_name" class="form-control @error('asset_name') is-invalid @enderror" 
                                    placeholder="Example: Asus A416EP" value="{{ old('asset_name', $assetDetail->asset_name) }}" required>
                                @error('asset_name')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="row m-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" name="description" 
                                    id="description" rows="3">{{ old('description', $assetDetail->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
                            ...
                            <div class="row m-3">
                                ...
                                <select id="category_id" name="category_id" class="form-control @error('category_id') is-invalid @enderror">
                                    @foreach($categories as $category)
                                        <option value="{{ $category['id'] }}" @if($category['id'] == old('category_id', $assetDetail->category_id)) selected @endif>
                                            {{ $category['category'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="row m-3">
                                ...
                                <select id="unit_id" name="unit_id" class="form-control @error('unit_id') is-invalid @enderror">
                                    @foreach($units as $unit)
                                        <option value="{{ $unit['id'] }}" @if($unit['id'] == old('unit_id', $assetDetail->unit_id)) selected @endif>
                                            {{ $unit['unit'] }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="row m-3">
                                ...
                                <textarea class="form-control @error('notes') is-invalid @enderror" name="notes" id="notes" 
                                    rows="3">{{ old('notes', $assetDetail->notes) }}</textarea>
                                @error('notes')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
